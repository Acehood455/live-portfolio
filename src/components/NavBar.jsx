...
export default function Navbar({ inNotch = false }) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setMobileMenuOpen((open) => !open)}
        className="fixed bottom-14 right-5 z-50 md:hidden shadow-[0px_4px_16px_0_rgba(211,187,54,.90)] bg-gold rounded-full p-3  text-text-950 hover:text-text-900 transition-colors duration-300"
        aria-label={mobileMenuOpen ? 'Close Menu' : 'Open Menu'}
      >
        {mobileMenuOpen ? <HiX size={28} /> : <HiMenu size={28} />}
      </button>
