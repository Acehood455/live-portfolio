...
const ProjectVideo = ({ project }) => {
  ...
  return (
    ...
    >
      {/* Stack pill */}
      <span className="absolute top-2 left-2 md:top-2 md:left-2 z-20 
        uppercase text-[9px] md:text-xs px-2 md:px-3 py-0.5 md:py-1 bg-background/50 md:bg-background/10 backdrop-blur
        text-text md:text-text
        border border-background/20 md:border-text/50 rounded-full"
      >
        {project.stack}
      </span>
        ...
        {projects.map((project) => (
          <SwiperSlide key={project.id} className="!h-full flex items-center justify-center">
            <motion.div
              ...
              initial="hidden"
              whileInView="visible"
              className={`m-4 h-[85%] md:h-[85%] z-20 bg-gradient-to-br ${project.gradient} rounded-2xl 
                shadow-xl p-4 md:p-6 flex flex-col md:flex-row gap-2`}
